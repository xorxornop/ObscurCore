...
namespace ObscurCore
{
    ...
    public static class Source
    {
        ...
        static Source() {
			...
			EngineInstantiatorsBlock.Add(SymmetricBlockCipher.Threefish, blockSize => ThreefishEngine.CreateCipher(blockSize));
			...
			EcParameters.Add(BrainpoolEllipticCurve.BrainpoolP384t1.ToString(), () => domainFuncGxy(
				...
				new BigInteger("18DE98B02DB9A306F2AFCD7235F72A819B80AB12EBD653172476FECD462AABFFC4FF191B946A5F54D8D0AA2F418808CC", 16), // x
				new BigInteger("25AB056962D30651A114AFD2755AD336747F93475B7A1FCA3B88F2B6A208CCFE469408584DC2B2912675BF5B9E582928", 16), // y
		...
		public static IMac CreateMacPrimitive (MacFunction macEnum, byte[] key, byte[] salt = null, byte[] config = null) {
			...
			if (macEnum == MacFunction.Hmac) {
				...
				if (config == null)
					throw new ArgumentException ("No block cipher specified (encoded as UTF-8 bytes).", "config");
				if (salt == null)
					throw new ArgumentException ("No IV supplied for the block cipher.", "salt");
				macObj = CreatePoly1305Primitive (Encoding.UTF8.GetString (config).ToEnum<SymmetricBlockCipher> (), key, salt);
			} else {
